...
from detect_utils.utils import log_set, get_image_list, make_args, to_mAP, video_load
...
# 封装画图函数到多线程样式
def multi_thread_visual_img(img_path_list, batch_size=1, img_result_path="./result_images", mAP_result_path="mAP/input/detection-results"):
...
def detect_images():
    ...
    preprocess_thread = threading.Thread(target=multi_thread_preprocess,
                                         args=(pre_process_batch_yolov7, args.img_list, args.batch_size, False))
...
def detect_video(write=False):
    ...
    tic = time.time()
    frame_shape, fps, num_frames, frame_list = video_load(args.video_path)
    toc = time.time()
    print(toc - tic)
    ...
    preprocess_thread = threading.Thread(target=multi_thread_preprocess, args=(pre_process_batch_yolov7, frame_list, 8, False))
    detection_thread = threading.Thread(target=multi_thread_detection)
